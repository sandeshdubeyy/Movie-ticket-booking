    ...
    <div className='px-6 md:px-16 lg:px-40 pt-30 md:pt-50'>
      <div className='flex flex-row m:flex-row gap-8 max-w-6xl mx-auto'>
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <p>
            {timeFormat(show.movie.runtime)} - {show.movie.genre.map(genre => genre.name).join(", ")} - {new Date(show.movie.release_date).toLocaleDateString("en-GB", {
              day: "numeric",
              month: "long",
              year: "numeric"
            })}
          ...
          <div className='flex items-center flex-wrap gap-4 mt-4'>
            ...
            <button onClick={handleFavorite} className='bg-gray-700 p-2.5 rounded-full transition cursor-pointer active:scale-95'>
              <Heart className={`w-5 h-5 ${favoriteMovies.find(movie => movie._id) ? 'fill-primary text-primary' : ""}`}/>
            </button>
          </div>
        </div>
      </div>
